...
- **service-input happy-path PDF test fixture.** PdfParser (oxidize-pdf
  2.x) has two error-path tests but no happy-path test — deferred until
  a known-good PDF fixture is available. Generate a minimal but valid
  PDF byte string (correct xref + object structure) and check it in as
  `tests/fixtures/minimal.pdf`. Then write a test in `pdf.rs` that reads
  the fixture, parses it, and asserts `text` is non-empty and `metadata
  ["page_count"]` >= 1. The fixture can be generated by the
  `generate_minimal_pdf.py` helper script or hand-crafted from the PDF
  specification (§7 of ISO 32000-1).
