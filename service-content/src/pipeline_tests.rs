...
fn pipeline_filter_graph_query_round_trip() {
    ...
    assert_eq!(entities.len(), 4, "filter must accept 4 and reject 2 noise entities");
    assert!(entities.iter().any(|e| e.entity_name == "Jennifer Woodfine" && e.classification == "Person"));
    assert!(entities.iter().any(|e| e.entity_name == "PointSav Digital Systems" && e.classification == "Company"));
    assert!(entities.iter().any(|e| e.entity_name == "project-totebox" && e.classification == "Project"));
    assert!(entities.iter().any(|e| e.entity_name == "Portugal" && e.classification == "Location"),
        "Portugal (Company→Location coercion) must be accepted with corrected classification");
    assert!(!entities.iter().any(|e| e.entity_name.contains("SLM_DATA_DIR")), "env var must be rejected");
    assert!(!entities.iter().any(|e| e.entity_name == "ops(slm)"), "parens expression must be rejected");
    ...
    assert_eq!(upserted, 4, "upserted count must match filtered entity count");

    // Query by name fragment returns the correct entity.
    let results = store
        .query_context(module_id, "Jennifer", 10)
        .expect("query_context");
    assert!(!results.is_empty(), "query_context must find 'Jennifer Woodfine'");
    ...
    assert_eq!(portugal[0].classification, "Location", "Portugal must be Location after coercion");
    ...
    assert_eq!(aliases, 0, "no ER aliases expected for 4 clearly distinct entities");
