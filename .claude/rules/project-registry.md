...
| service-fs | Active | service | activated 2026-04-25 (`ee209e3`); Tokio MCP-server skeleton `af73232`; SECURITY.md + ARCHITECTURE.md + RESEARCH.md `7d287d3`; **L2 trait extraction** `1e86047` (worm-ledger-design.md §5 step 1); **L1 PosixTileLedger** `10a7dd0` (worm-ledger-design.md §5 step 2 — persistent log.jsonl + D4 atomic-write + chain-tamper-detection on reload; trait grew by `checkpoint` + `verify_inclusion` + `verify_consistency` over linear SHA-256 chain; main.rs swapped to PosixTileLedger; `/v1/checkpoint` endpoint added; deps: sha2 + hex; **18 unit tests pass** — 11 trait-surface on InMemoryLedger + 7 PosixTileLedger durability/restart/tamper/file-mode); workspace `[members]` re-add deferred behind unrelated `openssl-sys` Layer 1 audit issue, currently in workspace `[exclude]`; next NEXT.md Right-now is step 3 checkpoint signing (Ed25519 + signed-note) |
| service-http | Scaffold-coded | service | 9 files |
| service-input | Active | service | created `fa1f71e` + activated `1490e27` (2026-04-25); **parser-dispatcher initial scaffold** `ada358d` (Format/ParsedDocument/ParseError/Parser trait/Dispatcher/detect_format; 11 tests); **PdfParser via oxidize-pdf 2.x** `<this commit>` (src/pdf.rs shims around oxidize-pdf's file-path-only API by writing input bytes to a uniquely-named temp file under std::env::temp_dir() with RAII Drop guard; returns ParsedDocument with extracted text + page_count metadata; 2 error-path tests pass — invalid bytes + malformed-magic; happy-path test deferred until known-good PDF fixture is added); **13 unit tests pass**; ADR-07 zero-AI throughout; workspace `[members]` re-add behind same openssl-sys Layer 1 audit blocker as service-fs, currently in workspace `[exclude]`; next pickup: MarkdownParser via pulldown-cmark |
