...
| service-fs | Active | service | activated 2026-04-25 (project-data Task Claude, `ee209e3`); Tokio MCP-server skeleton landed `af73232`; per-project SECURITY.md + ARCHITECTURE.md + RESEARCH.md landed `7d287d3`; **L2 LedgerBackend trait extraction** landed `1e86047` per worm-ledger-design.md §5 step 1 (factor `WormLedger` struct → `LedgerBackend` trait + `InMemoryLedger` impl, `http.rs` AppState holds `Box<dyn LedgerBackend + Send + Sync>`, 3 unit tests run against trait surface); 9 files under `service-fs/` (Cargo.toml + Cargo.lock + bilingual READMEs + CLAUDE.md + NEXT.md + ARCHITECTURE.md + SECURITY.md + RESEARCH.md + src/{main.rs, http.rs, ledger.rs}); workspace `[members]` re-add deferred behind unrelated `openssl-sys` Layer 1 audit issue, currently in workspace `[exclude]`; cargo check + cargo test pass standalone |
| service-http | Scaffold-coded | service | 9 files |
| service-input | Active | service | created and activated 2026-04-25 (`fa1f71e` + `1490e27`); **parser-dispatcher initial scaffold** landed `ada358d` 2026-04-26 — Cargo.toml (serde + serde_json today; format-specific parsers added as each is wired), src/lib.rs with Format enum + ParsedDocument struct + ParseError enum + Parser trait (object-safe Box<dyn Parser + Send + Sync>) + Dispatcher (per-format builder API + dispatch + dispatch_with_detection) + detect_format (extension-first, magic-byte fallback for PDF, ZIP-magic deferred to extension), 11 unit tests pass; ADR-07 zero-AI throughout; workspace `[members]` re-add behind same openssl-sys Layer 1 audit blocker as service-fs, currently in workspace `[exclude]`; next pickup: wire PdfParser via oxidize-pdf |
