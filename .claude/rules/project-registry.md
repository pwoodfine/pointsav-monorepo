...
| service-fs | Active | service | activated 2026-04-25 (`ee209e3`); **L2 trait** `1e86047`; **L1 PosixTileLedger** `10a7dd0` (18 tests); **step 3 checkpoint signing**: Ed25519Signer + signed Checkpoint; **step 4 audit-log sub-ledger**: second PosixTileLedger at `_audit/log.jsonl`; **round-trip integration test**: `tests/ledger_roundtrip.rs`; **MCP interface**: `src/mcp.rs` with `append_record` + `read_records` tools; **anchor-emitter** `anchor-emitter/` (commit `6262d10`): Sigstore Rekor v2 hashedrekord POST + tlog writeback, Doctrine Invention #7 Task half, 6 tests; **workspace [members]** (re-added 2026-04-27 by operator `b712641`); next: systemd unit file env-var surface confirm (Master-tier) |
...
| service-people | Active | service | activated 2026-04-25; **person-record schema** `src/person.rs` (2026-04-27): UUIDv5 from primary_email + builder pattern + 8 tests; **MCP server** `src/mcp.rs` + `src/http.rs` + `src/main.rs` + `src/fs_client.rs` + `src/people_store.rs` (commit `8c4eb7e`): `POST /mcp` with `identity.lookup` + `identity.append` tools; ureq 3.x FsClient → service-fs `/v1/append`; PeopleStore in-process index; 20 tests total; env: PEOPLE_MODULE_ID + PEOPLE_FS_URL + PEOPLE_BIND_ADDR (default 127.0.0.1:9300); next: FsClient end-to-end integration test |
