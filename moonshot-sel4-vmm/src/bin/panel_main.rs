...
//   console_pd formats a bordered cartridge panel with ANSI colors and sends
//   it as 24-byte IPC chunks. serial_pd receives chunks and writes each byte
//   to the debug serial port via SysDebugPutChar.
//
// Same IPC protocol as Phase H2 (serial_main.rs):
//   MR[0] = chunk byte count (0 = end-of-stream sentinel)
//   MR[1..3] = packed bytes, little-endian, up to 24 bytes per message
//
// Panel layout (42 chars wide):
//   +----------------------------------------+
//   |   os-console  Cartridge Status Panel   |
//   +----------------------------------------+
//   | F2  People       [ACTIVE]              |
//   | F3  Email        [ACTIVE]              |
//   | F4  Content      [ACTIVE]              |
//   | F6  Bookkeeper   [ACTIVE]              |
//   | F9  SLM/Doorman  [ACTIVE]              |
//   | F11 System       [ACTIVE]              |
//   | F12 Input        [ACTIVE]              |
//   +----------------------------------------+
//
// Gate: "Panel gate: PASSED" appears in QEMU serial output.
//
// Build:
//   CARGO_TARGET_DIR=/tmp/moonshot-h4-build cargo run \
//     --manifest-path moonshot-toolkit/Cargo.toml \
//     -- build moonshot-toolkit/examples/os-console-panel.toml
//
// Boot:
...
use moonshot_sel4_vmm::ansi;
use moonshot_sel4_vmm::bootinfo::{cap, msginfo_new, obj_type, BootInfo};
...
static mut ROOTSERVER_STACK: Stack = Stack([0; 8192]);
static mut SERIAL_PD_STACK:  Stack = Stack([0; 8192]);
static mut CONSOLE_PD_STACK: Stack = Stack([0; 8192]);
...
// Console PD: send ANSI-formatted cartridge panel via serial PD IPC.
unsafe fn console_pd_fn() -> ! {
    let ep = EP_SLOT;

    // Clear screen and home cursor.
    send_str(ep, ansi::CLEAR_HOME);

    // Box border line (42 chars wide).
    let border = b"+----------------------------------------+\r\n";
    let title  = b"|   os-console  Cartridge Status Panel   |\r\n";

    send_str(ep, border);
    send_str(ep, title);
    send_str(ep, border);

    // Each cartridge row: "| F##  Name         " + [ACTIVE] in green + "       |\r\n"
    // Column layout: | + space + F## + space + Name padded to 18 + status + pad + |
    let rows: &[(&[u8], &[u8])] = &[
        (b"| F2  People       ", b"                |\r\n"),
        (b"| F3  Email        ", b"                |\r\n"),
        (b"| F4  Content      ", b"                |\r\n"),
    ...
    for &(prefix, suffix) in rows {
        send_str(ep, prefix);
        send_str(ep, ansi::GREEN);
        send_str(ep, b"[ACTIVE]");
        send_str(ep, ansi::RESET);
        send_str(ep, suffix);
    }

    send_str(ep, border);

    // End-of-stream: serial_pd prints "Panel gate: PASSED" and spins.
    send_end(ep);
    spin()
}
...
pub unsafe extern "C" fn _main(bi: *const BootInfo) -> ! {
    write_bytes(b"[panel] Phase H4 start\r\n");

    let ipc = (*bi).ipc_buffer;
    let ipc_buf_addr = ipc as u64;

    // One EP (16B) + two TCBs (2 KiB each) = ~4 KiB; use 8 KiB untyped.
    let ut = match (*bi).find_untyped(13) {
        Some(s) => s,
        None => {
            write_bytes(b"[panel] no untyped >= 8 KiB\r\n");
            spin()
        }
    };
    ...
    // serial_pd (200) > console_pd (100) > rootserver (0).
    ...
    let serial_sp = core::ptr::addr_of!(SERIAL_PD_STACK)  as u64 + 8192;
    ...
    let console_sp = core::ptr::addr_of!(CONSOLE_PD_STACK) as u64 + 8192;
    ...
    // serial_pd must block in Recv before console_pd starts sending.
