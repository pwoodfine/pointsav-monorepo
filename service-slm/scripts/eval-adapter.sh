...
echo "        then run scripts/lora-scaled-dropin.sh --adapter-path ${ADAPTER_DIR} --apply"
echo "        (writes a systemd drop-in; operator still runs the actual"
echo "        'systemctl restart local-slm' — see BRIEF-flow-quality-audit.md"
echo "        for the full promotion-path writeup, incl. why this pass@5 score"
echo "        alone shouldn't be the only gate — see deploy-gate.sh/score-gate.sh)"
echo ""
